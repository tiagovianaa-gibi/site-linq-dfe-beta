...
function buildNoticiaPage(item) {
  ...
  const image = toSitePath(item.imagemCard || item.imagemHeroUrl || item.imagem, "/assets/banners/placeholder.jpg");
  const socialImage = toAbsoluteSiteUrl(item.imagemCard || item.imagemHeroUrl || item.imagem, OG_IMAGE);
